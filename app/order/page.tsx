    ...
    <div className="relative">
      <button
        ...
        className={`w-full flex items-center justify-between bg-[oklch(0.16_0.01_250)] border rounded-xl px-4 py-3 text-sm outline-none transition-colors ${
          hasError ? 'border-destructive/60' : 'border-border hover:border-white/20 focus:border-primary/70'
        ...
        <div className="flex flex-col gap-3">
          {[
            ...
            <a key={c.href} href={c.href} className="flex items-center gap-3 group">
              ...
              <div>
                <p className="text-foreground font-semibold text-sm group-hover:text-primary transition-colors">{c.number}</p>
  ...
  return (
    ...
    >
      ...
      <p className="text-muted-foreground leading-relaxed mb-8 max-w-sm mx-auto">
        WhatsApp открыт с вашим сообщением. После отправки наш менеджер свяжется с вами в ближайшее время.
      </p>
      <button
        onClick={onReset}
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
        <div>
          ...
          <input type="text" placeholder="ФИО или название компании" value={form.fullName} onChange={set('fullName')} className={inputCls(!!errors.fullName)} />
        ...
        <div>
          ...
          <input type="tel" placeholder="+7 707 000 00 00" value={form.phone} onChange={set('phone')} className={inputCls(!!errors.phone)} />
      ...
      <div className="mb-5">
        ...
        <Dropdown
          ...
          value={form.material}
          onChange={(v) => { setForm((p) => ({ ...p, material: v })); setErrors((p) => ({ ...p, material: undefined })) }}
          placeholder="Выберите материал или услугу"
          hasError={!!errors.material}
        />
        {errors.material && <p className="text-destructive text-xs mt-1">{errors.material}</p>}
      </div>
      ...
      <div className="mb-5">
            ...
            {(['тонн', 'м³'] as UnitType[]).map((u) => (
              <button
                ...
                className={`px-4 py-3 text-sm font-semibold transition-colors ${
                  form.unit === u ? 'bg-primary text-primary-foreground' : 'bg-[oklch(0.16_0.01_250)] text-muted-foreground hover:text-foreground'
                }`}
              >
                {u}
              </button>
        ...
        <p className="text-muted-foreground text-xs mt-1.5">Например: 4 тонны щебня или 3 м³ песка</p>
      </div>
      ...
      <div className="mb-5">
        ...
        <input type="text" placeholder="г. Алматы, ул. Примерная, 10" value={form.address} onChange={set('address')} className={inputCls(!!errors.address)} />
      ...
      <div className="mb-8">
        <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">
          Комментарий <span className="ml-2 text-muted-foreground/50 normal-case font-normal">(необязательно)</span>
        </label>
        <textarea rows={3} placeholder="Дополнительные пожелания, особенности подъезда..." value={form.comment} onChange={set('comment')} className={`${inputCls(false)} resize-none`} />
      </div>

      <button type="submit" className="w-full flex items-center justify-center gap-3 px-8 py-4 rounded-full bg-[oklch(0.38_0.15_155)] hover:bg-[oklch(0.42_0.17_155)] text-white font-bold text-base transition-all hover:scale-[1.02] active:scale-95 shadow-lg">
    ...
    >
      ...
      <div className="mb-5">
        ...
        <Dropdown
          ...
          onChange={(v) => { setForm((p) => ({ ...p, cargoType: v })); setErrors((p) => ({ ...p, cargoType: undefined })) }}
          placeholder="Выберите тип груза"
          hasError={!!errors.cargoType}
        />
        {errors.cargoType && <p className="text-destructive text-xs mt-1">{errors.cargoType}</p>}
      </div>
      ...
      <div className="mb-5">
        ...
        <p className="text-muted-foreground text-xs mt-1.5">Минимальный заказ — 5 тонн</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
        <div>
          ...
          <input type="text" placeholder="г. Алматы, ул. Складская, 5" value={form.from} onChange={set('from')} className={inputCls(!!errors.from)} />
      ...
      <div className="mb-8">
        ...
        <textarea rows={3} placeholder="Особенности груза, требования к транспорту, желаемое время..." value={form.comment} onChange={set('comment')} className={`${inputCls(false)} resize-none`} />
      </div>
...
function OrderPageInner() {
  ...
  useEffect(() => { setSubmitted(false) }, [activeTab])
            ...
            <div className="lg:col-span-2">
              <AnimatePresence mode="wait">
                {submitted ? (
                  ...
                  <CargoFormSection key="cargo" onSuccess={() => setSubmitted(true)} />
                )}
              </AnimatePresence>
            </div>
